...
//   var elements    = {};   // id → DOM avatar element (for photo injection)
...
//   /* Build marker icon – avatar is a div with text initially */
//   function mkIcon(s,sel){
//     var sz   = sel?54:46;
//     var bubW = sel?(sz+6+Math.min(s.name.length*8,130)+20):(sz+6);
//     var label= sel?('<span class="mk-label">'+s.name+'</span>'):'';
//     /* Give the avatar div a unique id so we can swap it for an img later */
//     var avatarId='av_'+s.id+(sel?'_s':'');
//     var avatarHtml='<div id="'+avatarId+'" class="mk-avatar" '
//       +'style="background:'+s.color+';width:'+sz+'px;height:'+sz+'px">'
//       +s.init+'</div>';
//     var html='<div class="mk">'
//       +'<div class="mk-bubble'+(sel?' sel':'')+'">'+avatarHtml+label+'</div>'
//       +'<div class="mk-pin" style="border-top-color:'+(sel?'#CDE1E2':'#fff')+'"></div>'
//       +'</div>';
//     return L.divIcon({html:html,className:'',iconSize:[bubW,74],iconAnchor:[bubW/2,72]});
//   }
...
//       markers[id].setZIndexOffset(1000);
//       map.panTo([s.lat,s.lon],{animate:true,duration:.5});
//       /* Re-apply photo if available */
//       if(elements[id]) applyPhoto(id, elements[id], 54);
//     }
//     window.ReactNativeWebView&&
//       window.ReactNativeWebView.postMessage(JSON.stringify({type:'select',id:id}));
//   }
...
//     /* photoMap: { "student_id": "data:image/jpeg;base64,..." } */
//     Object.keys(photoMap).forEach(function(sid){
//       var id=parseInt(sid,10);
//       elements[id]=photoMap[sid];
//       applyPhoto(id,photoMap[sid],46);
//     });
//   };

//   function applyPhoto(id,src,sz){
//     /* Find the avatar element in the live DOM and swap to <img> */
//     var suffix=(selectedId===id)?'_s':'';
//     var el=document.getElementById('av_'+id+suffix);
//     if(el&&src){
//       el.innerHTML='';                          /* clear initials text */
...
//       img.onerror=function(){el.textContent=el.dataset.init||'?'};
//       el.appendChild(img);
//     }
//   }
...
//       {/* Native header floats above WebView */}
//       <View style={styles.header}>
//         <TouchableOpacity
//           style={styles.backBtn}
//           onPress={() => navigation.goBack()}
